...
const int g_nEvSave = g_nEvent/2; // save checkpoint every now and then
...
const int g_nZ = 28; // proton number
const int g_nAMass = 70; // proton + neutron number
const int g_nDisLvlMax = 6; // only trust level scheme to here, sets ECrit
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
...
#ifdef bLD_CTM
  const double g_dTemp =  0.922; // MeV
  const double g_dE0   =  0.788; // MeV - robin says 0.788 but I'm trying higher NLD
  ...
  const double g_dDeuPair = 3.583; // MeV; //robin from EB03 mass table
  #endif
...
#include "GSFTable.dat" // just placeholder values in this file for now
  ...
  //#define bE1_UsrDef // user defined   // these just match what I had in dicebox
  const double g_adSigE1[] = {190.00, 0.00}; // mb magnitude
  const double g_adEneE1[] = { 17.50, 0.00}; // MeV centroid energy, non-zero
  const double g_adGamE1[] = {  8.40, 0.00}; // MeV GDR width
  //                                  ^^^^ for a 2nd resonance
...
const double g_dExIMax = 9.; // MeV; above max population energy
const char popFile[] = "population/70Ni_pop_combined.dat"; // made from TALYS "outpopulation y"
// make sure to match # of discrete bins. See ReadPopFile() bins + maxlevelstar + 1 = g_nExPopI
const int g_nExPopI = 76; // contbins + maxlevelstar + 1 = g_nExPopI
const int g_nSpPopIBin = 10; // spins 0-9
const double g_dExRes = 0.0; // excitation resolution on g_ah2ExEg
